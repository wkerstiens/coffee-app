...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole{
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "The caller could not be verified"); 
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "The amount paid is not greter than or equal to the price"); 
    _;
  }
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "This item has not been harvested or it has already been processed");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(items[_upc].itemState == State.Processed, "This item has not been processed or it has already been packed.");
    _;
  }
  
  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, "This item has not been packed or it has already for sale.");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is ForSale
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, "This item is not up for sale");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Sold
  modifier sold(uint _upc) {
    require(items[_upc].itemState == State.Sold, "This item has not been sold or it has already shipped.");
    _;
  }
  
  // Define a modifier that checks if an item.state of a upc is Shipped
  modifier shipped(uint _upc) {
    require(items[_upc].itemState == State.Shipped, "THis item has not been shipped or it has already been received");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
    require(items[_upc].itemState == State.Received, "This item has not been received or it has already been purchased");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint _upc) {
    require(items[_upc].itemState == State.Purchased, "This itme has not been purchased");
    _;
  }
